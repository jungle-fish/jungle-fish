...
function HeroContent() {
  const { t } = useLanguage();
  const { isReady } = useHeroIntro();
  const { scrollY } = useScroll();

  const bgY = useTransform(scrollY, [0, 1200], [0, 1200]);
  const bgScale = useTransform(scrollY, [0, 1200], [1, 2]);
  const overlayDarkness = useTransform(scrollY, [0, 1000], [0, 1]);
  const contentY = useTransform(scrollY, [0, 900], [0, 625]);
  ...
  return (
    <section className="relative min-h-[92vh] overflow-hidden">
      ...
      <motion.div
        style={{ opacity: isReady ? overlayDarkness : 0 }}
      ...
      <Container className="relative z-10 flex min-h-[92vh] flex-col justify-center py-28">
        ...
        >
          <motion.p
            ...
            transition={{ duration: 0.7, delay: 0.1, ease: CONTENT_EASE }}
          ...
          <motion.h1
            ...
            transition={{ duration: 0.8, delay: 0.2, ease: CONTENT_EASE }}
          ...
          <motion.p
            initial={{ opacity: 0, y: 24 }}
            animate={isReady ? { opacity: 1, y: 0 } : { opacity: 0, y: 24 }}
            transition={{ duration: 0.8, delay: 0.35, ease: CONTENT_EASE }}
          ...
          <motion.div
            initial={{ opacity: 0, y: 24 }}
            animate={isReady ? { opacity: 1, y: 0 } : { opacity: 0, y: 24 }}
            transition={{ duration: 0.8, delay: 0.5, ease: CONTENT_EASE }}
        ...
        <motion.a
          ...
          transition={{ delay: isReady ? 1 : 0, duration: 0.6, ease: CONTENT_EASE }}
          className="absolute bottom-8 left-1/2 flex -translate-x-1/2 flex-col items-center gap-1 text-jungle-200/80 transition-colors hover:text-white"
